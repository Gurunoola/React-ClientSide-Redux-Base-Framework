...
export default function Users(props) { //change for new component
  ...
  const columns = [
    ...
    { accessorFn: (row) => `${_.capitalize(row.firstName)} ${_.capitalize(row.lastName)}`, header: labels.NAME, size: defaultColSize },
    { accessorKey: 'email', header: labels.EMAIL, size: 250 },
    { accessorKey: 'role', header: labels.ROLE, size: 250 },
    { accessorFn: (row) => `${row.isEmailVerified}`,
    header: `${labels.EMAIL_VERIFIED}`,
    //Add a link in a cell render
    Cell: ({ renderedCellValue, row }) => (
      <span class={`badge text-white ${row.isEmailVerified ? 'bg-success' : 'bg-danger'}`}>{renderedCellValue}</span>
    ),}
  ]
