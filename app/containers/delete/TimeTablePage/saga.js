...
const doGetAll = options => {
  const params = `?limit=${options.options.pageLimit || 10}&page=${options
    .options.page || 1}&populate=subject,class,teacher`;
...
const findBy = options => {
  const params = `/findByQuery?class=${options.query}`;
  return doServerRequest({ url: `${url}${params}`, method: 'get' });
};

function* findByQuery(params) {
  const { response, error } = yield call(findBy, params);
  const { event, message } = constructEvents(response, error, 'find', componentNameCaps);
  const data = _.get(response, 'data', []);
  const rest = _.omit(_.get(response, 'data', []), 'results');
  yield put({ type: event, result: {data: [...data], ...rest} || undefined, message });
}

export default function* timetablesSaga() {
  yield all([
    ...
    takeLatest(LIST_GET_FIND_BY_QUERY_REQUESTED, findByQuery)
  ]);
}
