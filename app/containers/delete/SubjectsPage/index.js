...
export default function Subjects(props) { //change for new component
  ...
  const columns = [
    ...
    { accessorFn: (row) => `${row.name}`, header: labels.SUBJECT_NAME, size: defaultColSize },
    { accessorKey: 'department', header: labels.DEPARTMENT, size: 250 },
    { accessorKey: 'subjectCode', header: labels.SUBJECT_CODE, size: defaultColSize },
    {
      accessorFn: (row) => { return row.classes.map(data => { return `${data.name + data.section}` }) },
      header: `${labels.CLASS}`,
      Cell: ({ renderedCellValue, row }) =>
        renderedCellValue && renderedCellValue.map((item) => {
          return <span className=" rounded-0 badge p-1 m-1 text-gray bg-gray-300" style={{ fontSize: "12px" }}>{item}</span>
        })
    }
  ]
  useEffect(() => {
  ...
  const onSubmit = (values, newUser = false) => {
    ...
    const classes = values.classes.map(p => p.value);
    delete newData.classes
    const finalData = { ...newData, classes: [...classes] }
    if (newUser) {
      post({ ...finalData })
    } else {
