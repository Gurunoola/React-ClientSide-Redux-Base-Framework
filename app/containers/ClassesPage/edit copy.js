...
export function Edit({ showProgressBar, data, title, id, onView, onSubmit }) {
  ...
  const actionButtons = [
    {
      ...
      type: '',
      onClick: (event) => {
        document.getElementById('formClass').dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }))
      }
    },
    {
  ...
  function getForm() {
    return  <>
    <form id="formClass" onSubmit={handleSubmit((data) => { showProgressBar(true); onSubmit(data, isNewUser) })}>
